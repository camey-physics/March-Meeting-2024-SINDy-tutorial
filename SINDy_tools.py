...
# def get_random_sample(numSamples, lx, ly, lt, wx=0, wy=0, wt=0):
    
#     xx = np.arange(0, lx)
#     yy = np.arange(0, ly)
#     tt = np.arange(0, lt)
        
#     X, Y, T = np.meshgrid(xx, yy, tt, indexing='ij', sparse=True)
#     X = X[:,None,None]; Y = Y[None,:,None]; T = T[None,None,:]
#     ind = (X >= wx) *(X < (lx-wx)) *(Y >= wy) *(Y < (ly-wy)) *(T >= wt) *(T < (lt-wt))
    
#     # xi = np.random.randint(wx+1, lx-wx-1, num)
#     # yi = np.random.randint(wy+1, ly-wy-1)
#     # ti = np.random.randint(wt+1, lt-wt-1)
